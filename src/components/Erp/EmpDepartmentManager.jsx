...
import { ListGroup, ListGroupItem, ListGroupItemHeading, ListGroupItemText, Table,
    Breadcrumb, BreadcrumbItem, Modal, ModalBody, ModalFooter, ModalHeader } from "reactstrap";
...
const EmpDepartmentManager = () => {
    ...
    const toggle = (index) => {
        setEmp(emp.map((item, i) => (i === index ? {...item, isOpen: !item.isOpen} : item)));
        emp.forEach((item, i) => (
            i === index ? loadEmpProfileImg(item.empno): null)
        )      
        console.log("toggle emp: ", emp);
    }
    ...
    return(
        <ListGroupItem>
                ...
                <Table borderless size="sm" className='f-s-95'>
                    ...
                    {emp.map((item, index) => (
                        <React.Fragment key={item.EMPNO}>                        
                            ...
                            <Modal isOpen={item.isOpen} fade={true} toggle={() => toggle(index)}>
                                <ModalHeader>
                                    {item.name}님 프로필
                                </ModalHeader>
                                <ModalBody>
            ...
            <Breadcrumb tag='nav' listTag="div" className="float-right">                        
                <BreadcrumbItem tag="a" href="#top">상단으로</BreadcrumbItem>
            </Breadcrumb>       
        </ListGroupItem>
    )
}

export default EmpDepartmentManager;